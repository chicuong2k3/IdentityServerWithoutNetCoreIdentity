...
public class Callback : PageModel
{
    ...
    private readonly Dictionary<string, string> _googleClaimTypeMap = new Dictionary<string, string>
    {
        { "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress", JwtClaimTypes.Email },
        { "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name", JwtClaimTypes.Name },
        { "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/givenname", JwtClaimTypes.GivenName },
    ...
    private readonly Dictionary<string, string> _azureClaimTypeMap = new Dictionary<string, string>
    {
    ...
    public async Task<IActionResult> OnGet()
    {
        ...
        if (user == null)
        {
            ...
            if (provider == "Google")
            {
                var emailFromGoogle = claims.FirstOrDefault(x => x.Type == "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress")?.Value;

                if (!string.IsNullOrEmpty(emailFromGoogle))
                {
                    user = await _localUserService.GetUserByEmailAsync(emailFromGoogle);

                    if (user != null)
                    {
                        await _localUserService.AddExternalProviderToUserAsync(user.Subject, provider, providerUserId);
                    ...
                    else
                    {
                        ...
                        // add custom claims for user
                        // can add via registration page
                        // here we add fixed claims
                        mappedClaims.Add(new Claim("role", "Customer"));
                        mappedClaims.Add(new Claim("country", "vn"));

                        user = _localUserService.AutoProvisionUser(provider, providerUserId, mappedClaims);
                        await _localUserService.SaveChangesAsync();
                    }
                }
            } 
            else if (provider == "AAD")
            {
                ...
                foreach (var claim in claims)
                {
                    if (_azureClaimTypeMap.ContainsKey(claim.Type))
                    {
                        mappedClaims.Add(new Claim(_azureClaimTypeMap[claim.Type], claim.Value));
                    }
                }
